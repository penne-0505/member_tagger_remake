    ...
    def format_result(
        ...
        result: dict[
            ...
            list[
                dict[
                    str,
                    discord.User | list[dict[str, datetime.datetime | discord.Thread]],
                ]
            ],
        ],
    ) -> str:
        ...
        for mode, data in result.items():
            if mode == "get_users_by_thread":
                ...
                for user_data in data:
                    ...
                    threads = (
                        threads[str(current_guild.id)]
                        if str(current_guild.id) in list(threads.keys())
                        else []
                    )
    ...
    def get_embed(self, data: dict[str, Tag | Task]) -> discord.Embed:
        ...
        if current_mode == "tag":
            if not data["tag"].thread_id:
                ...
                data["tag"].guild_id and data["tag"].thread_id and not data["tag"].users
            ):
                embed = discord.Embed(
                    title="** 2/3 ** メンバーの選択",
                ...
                data["tag"].guild_id
                and data["tag"].thread_id
                and data["tag"].users
                and data["tag"].deadline
            ):
                embed = discord.Embed(
                    title="3/3 タグ付け完了",
            ...
            if not data["untag"].thread_id:
                ...
                data["untag"].guild_id
                and data["untag"].thread_id
                and not data["untag"].users
            ):
                tagged_user_ids = data["untag_tagged_user_ids"]
                tagged_users = [
                    data["untag"].client.get_user(int(user_id.id))
            ...
            if not data["get_threads_by_user"].users:
                ...
                embed = discord.Embed(
                    title="取得結果: ", description=result, color=discord.Color.green()
                )
                return embed
            ...
            embed = discord.Embed(
                title="リダイレクトが完了しました", color=discord.Color.green()
            )
            return embed
            ...
            result = self.format_result(data["result"])
            embed = discord.Embed(
                title="通知", description=result, color=discord.Color.green()
            )
            return embed
